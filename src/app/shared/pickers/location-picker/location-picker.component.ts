...
export class LocationPickerComponent implements OnInit {
      ...
      .then((modalEl) => {
        modalEl.onDidDismiss().then((modalData) => {
          ...
          this.getAddress(modalData.data.lat, modalData.data.lng).pipe(
            ...
            pickedLocation.staticMapImageUrl = mapUrl;
            this.selectedLocationImage = mapUrl;
  ...
  private getMapImage(lat: number, lng: number, zoom: number){
    ...
    &markers=color:red%7Clabel:Places%7C${lat},${lng}&key=${environment.GOOGLE_MAPS_API_KEY}`;
  }
}
